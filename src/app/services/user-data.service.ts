...
export class UserDataService {
  ...
  public userResponses$: Observable<UserResponse[]> = this.userResponsesSubject.asObservable();
  ...
  private loadUserResponses(): void {
    try {
      ...
      console.error('Erreur lors du chargement des réponses utilisateur:', error);
    }
  }
  ...
  private saveUserResponses(): void {
    try {
      ...
      console.error('Erreur lors de la sauvegarde des réponses utilisateur:', error);
    }
  }
   ...
   */
  saveResponse(moduleId: string, questionId: string, response: string | string[] | number | boolean): void {
  ...
  getModuleResponses(moduleId: string): UserResponse[] {
    return this.userResponsesSubject.value.filter(r => r.moduleId === moduleId);
  }
